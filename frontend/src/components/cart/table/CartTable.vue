...
const { cartProducts, deleteHandler } = defineProps<{ cartProducts: CartProduct[], deleteHandler: Function }>();
...
const columns: ColumnDef<CartProduct, any>[] = [
  ...
  {
    id: "action",
    cell: ({ row }) => h(FontAwesomeIcon, { icon: faTrash, size: "lg", style: {color: "#67778d", cursor: "pointer"}, onClick: () => deleteHandler(row.original.id) }),
  }
];
